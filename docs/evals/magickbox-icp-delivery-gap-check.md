...
Latest update: 2026-05-23T18:47:13+07:00
...
- A separate MagickBoxV3 app is live through the Caffeine.ai ICP builder at `https://magickbox-icp-e68.caffeine.xyz/`; version 9 serves certified frontend assets from asset canister `i2fwa-kyaaa-aaaam-qizja-cai` and connects to backend/core canister `itg54-4qaaa-aaaam-qiziq-cai` through `/env.json`.
- Live MagickBoxV3 v9 bundle smoke confirmed the product name, build marker `magickboxv3-ii-authorize-20260523`, and the correct Internet Identity authorize endpoint `https://id.ai/authorize`.
...
| --- | --- | --- |
| Mainnet ICP canisters | MagickBoxV3 builder preview has a live certified asset canister and backend canister; direct non-builder mainnet preflight remains blocked by 0 ICP/0 cycles on `magickbox-mainnet-isolated`. | Claim owner on the isolated MagickBoxV3 preview with Mark's II, then separately fund the dedicated isolated identity if a direct non-builder deployment is still required. |
| Isolated web preview | MagickBoxV3 v9 is live at `https://magickbox-icp-e68.caffeine.xyz/`, loads real backend config from `/env.json`, and opens Internet Identity through `https://id.ai/authorize`. | Mark must claim the one-time superadmin role with II before funding or broad sharing. |
